...
class education implements ActionListener{
	JFrame fr;
	JPanel p11,p12,p13;
	...
	education()throws Exception{
		fr = new JFrame("学历信息管理");
		fr.setSize(1365, 725);
		p11 = new JPanel();
		p12 = new JPanel();
		p13 = new JPanel();
		p11.setLayout(new GridLayout(6,2,0,100));
		l1 = new JLabel("毕业证书编号");
...
sql = "select * from 学历信息表";//SQL语句  
	...
	public void actionPerformed(ActionEvent e){
		if(e.getSource()==b1){
			...
			lr.add(tf3.getText());
			lr.add(tf4.getText());
			lr.add(cbb5.getSelectedItem());
			lr.add(tf6.getText());
			
			
			tam.addRow(lr);
			try{
				db1 = new DBHelper("insert into 学历信息表 values ('"+tf1.getText()+"','"+tf2.getText()+"','"+tf3.getText()+"','"+tf4.getText()+"','"+cbb5.getSelectedItem()+"','"+tf6.getText()+"')");//创建DBHelper对象
		...
		else if(e.getSource()==b2)
		{
			...
			case 4:
				...
				ta.setModel(tam);
				try{
					db1 = new DBHelper("update 学历信息表 set 专业='"+tf4.getText()+"' where 员工编号 = '"+id+"'");//创建DBHelper对象
				
					}catch(Exception e1){
					e1.printStackTrace();
				}break;
			case 5:
				tam.setValueAt(cbb5.getSelectedItem(), selectRow, selectColumn);
				ta.setModel(tam);
				try{
					db1 = new DBHelper("update 学历信息表 set 毕业时间='"+cbb5.getSelectedItem()+"' where 员工编号 = '"+id+"'");//创建DBHelper对象
			...
			case 6:
				...
				ta.setModel(tam);
				try{
					db1 = new DBHelper("update 学历信息表 set 毕业院校='"+tf6.getText()+"' where 员工编号 = '"+id+"'");//创建DBHelper对象
		...
		else if(e.getSource()==b3)
		{
			...
			ta.setModel(tam);
			try{
				db1 = new DBHelper("delete from 学历信息表 where 毕业证书编号 = '"+id+"'");//创建DBHelper对象
